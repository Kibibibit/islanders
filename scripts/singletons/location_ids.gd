...
## Checks if a location ID is an apartment in the main building.
static func is_islander_apartment(loc_id: int) -> bool:
	return loc_id & ISLANDER_APARTMENT_MASK > 0

## Checks if a location ID is a house in the housing area, whatever that ends up looking like.
static func is_islander_house(loc_id: int) -> bool:
	return loc_id & ISLANDER_HOUSE_MASK > 0

## Checks if a location ID belongs to an islander.
